...
- **OpenWatchParty `0.3.0`** targets Jellyfin ABI `10.11.0.0`
...
docker run -d \
  ...
  ghcr.io/mhbxyz/owp-session-server:0.3.0
```
...
docker run -d \
  ...
  -e ALLOWED_ORIGINS="http://localhost:8096" \
  -e JWT_SECRET="$JWT_SECRET" \
  ghcr.io/mhbxyz/owp-session-server:v0.3.0
...
1. **Download the Plugin**
   ...
   ```bash
   curl -fLO https://github.com/mhbxyz/OpenWatchParty/releases/download/v0.3.0/OpenWatchParty-v0.3.0.zip
   ```
...
2. **Install to Jellyfin**
   ...
   ```bash
   # Linux (Docker)
   unzip OpenWatchParty-v0.3.0.zip -d /tmp/owp
   docker cp /tmp/owp/. jellyfin:/config/plugins/OpenWatchParty/

   # Linux (native)
   sudo unzip OpenWatchParty-v0.3.0.zip -d /var/lib/jellyfin/plugins/OpenWatchParty/
