...
OpenWatchParty `0.3.4` (Jellyfin 10.11) and `0.4.0` (Jellyfin 12) send the modern `Authorization` header. `0.3.3` and earlier do not, so they cannot authenticate against Jellyfin 12.

**Solutions:**

1. **Upgrade OpenWatchParty to `0.3.4` or `0.4.0`**
   - This is the supported fix. Install the release matching your Jellyfin version from the plugin repository or manually.
